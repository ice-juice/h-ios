...
NSString *const AMControllerDidAddToSuperViewNotification = @"AMControllerDidAddToSuperViewNotification";
NSString *const AMControllerDidFullyDisplayedNotification = @"AMControllerDidFullyDisplayedNotification";

static NSInteger const kWMUndefinedIndex = -1;
static NSInteger const kWMControllerCountUndefined = -1;
@interface AMPageController () {
...
@implementation AMPageController
...
- (void)dealloc {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(am_growCachePolicyAfterMemoryWarning) object:nil];
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(am_growCachePolicyToHigh) object:nil];
}
...
- (void)wm_postAddToSuperViewNotificationWithIndex:(int)index {
    ...
    //2020-09-10
    //[[NSNotificationCenter defaultCenter] postNotificationName:AMControllerDidAddToSuperViewNotification
                                                        //object:info];
    [[NSNotificationCenter defaultCenter] postNotificationName:AMControllerDidAddToSuperViewNotification object:self userInfo:info];
}
...
- (void)wm_postFullyDisplayedNotificationWithCurrentIndex:(int)index {
    ...
    //2020-09-10
//    [[NSNotificationCenter defaultCenter] postNotificationName:AMControllerDidFullyDisplayedNotification
//                                                        object:info];
    [[NSNotificationCenter defaultCenter] postNotificationName:AMControllerDidFullyDisplayedNotification object:self userInfo:info];
}
...
- (void)am_growCachePolicyAfterMemoryWarning {
    self.cachePolicy = WMPageControllerCachePolicyBalanced;
    [self performSelector:@selector(am_growCachePolicyToHigh) withObject:nil afterDelay:2.0 inModes:@[NSRunLoopCommonModes]];
}

- (void)am_growCachePolicyToHigh {
...
- (void)didReceiveMemoryWarning {
    ...
    if (self.memoryWarningCount < 3) {
        [self performSelector:@selector(am_growCachePolicyAfterMemoryWarning) withObject:nil afterDelay:3.0 inModes:@[NSRunLoopCommonModes]];
    }
}

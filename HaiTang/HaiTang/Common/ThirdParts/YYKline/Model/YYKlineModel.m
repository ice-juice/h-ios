...
- (NSAttributedString *)V_Price {
    if (!_V_Price) {
        //2020-11-30
        _V_Price = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %@  开：%.2f  高：%.2f 低：%.2f  收：%.2f ", self.V_Date, self.open.floatValue, self.high.floatValue, self.low.floatValue, self.close.floatValue] attributes:@{
...
- (NSAttributedString *)V_MA {
    if (!_V_MA) {
        //2020-11-30
        NSAttributedString *str1 = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %@  开：%.2f  高：%.2f 低：%.2f  收：%.2f ", self.V_Date, self.open.floatValue, self.high.floatValue, self.low.floatValue, self.close.floatValue] attributes:@{
            NSForegroundColorAttributeName: [UIColor grayColor],
        }];
...
- (NSAttributedString *)V_Volume {
    if (!_V_Volume) {
        _V_Volume = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" 成交量：%.0f", self.vol.floatValue] attributes:@{
            NSForegroundColorAttributeName: [UIColor grayColor],
        }];
    }
    return _V_Volume;
